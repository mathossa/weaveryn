...
export default async function WorldOverviewPage({
  ...
  const ownerPreviewCount = world.accessKind === 'OWNER' ? 1 : 0
  const memberPreview = (worldMembers ?? []).slice(0, 4 - ownerPreviewCount)
  const hiddenMemberCount = Math.max(
    0,
    world.memberCount - ownerPreviewCount - memberPreview.length,
  )
  ...
  return (
    ...
    >
      <main
        className={`${styles.dashboard} ${dashboardStyles.dashboardSpacing}`}
      >
        ...
        <div className={dashboardStyles.dashboardGrid}>
            ...
            <div className={styles.heroContent}>
              ...
              {world.canEditBasicInfo ? (
                <div className={dashboardStyles.heroActions}>
                  <Link
                    className={dashboardStyles.heroEdit}
          ...
          <section
            className={`${styles.sidebarPanel} ${dashboardStyles.actionsArea}`}
          >
          ...
          <section
            className={`${styles.dashboardPanel} ${dashboardStyles.campaignsArea}`}
          >
          ...
          <section
            className={`${styles.sidebarPanel} ${dashboardStyles.overviewArea}`}
          >
          ...
          <section
            className={`${styles.dashboardPanel} ${dashboardStyles.mapPanel} ${dashboardStyles.mapArea}`}
          >
            <div className={styles.sectionHeader}>
              <div>
                <span className={styles.sectionKicker}>World map preview</span>
                <h2>World map</h2>
              </div>
              <span className={dashboardStyles.soonBadge}>Soon</span>
            </div>
            <div className={dashboardStyles.mapPlaceholder}>
              <span className={dashboardStyles.mapGlyph} aria-hidden="true">
                ⌖
              </span>
              <div className={dashboardStyles.mapCopy}>
                <strong>No map woven yet</strong>
                <p>
                  World maps will visualize continents, regions, and important
                  places from this World in one workspace.
                </p>
              </div>
            </div>
          </section>
          ...
          <section
            className={`${styles.sidebarPanel} ${dashboardStyles.accessArea}`}
          >
            <div className={dashboardStyles.accessHeader}>
              <div className={dashboardStyles.accessHeading}>
                <span className={styles.sidebarTitle}>Access</span>
                <span className={dashboardStyles.accessCount}>
                  World members · {world.memberCount}
                </span>
              </div>
              <span className={dashboardStyles.accessIcon} aria-hidden="true">
                ◉
              </span>
            </div>
            ...
            {world.canManageMembers ? (
              <div
                className={`${styles.memberList} ${dashboardStyles.compactMemberList}`}
              >
                {world.accessKind === 'OWNER' ? (
                  <div
                    className={`${styles.memberRow} ${dashboardStyles.compactMemberRow}`}
                  >
                    <span
                      className={`${styles.memberAvatar} ${dashboardStyles.compactMemberAvatar}`}
                    >
                      YOU
                    </span>
                    <span
                      className={`${styles.memberIdentity} ${dashboardStyles.compactMemberIdentity}`}
                    >
                      <strong>You</strong>
                    </span>
                ...
                {hiddenMemberCount > 0 ? (
                  <Link
                    className={dashboardStyles.moreMembersRow}
                    href={`/world/${world.id}/members`}
                  >
                    <span>
                      + {hiddenMemberCount} more{' '}
                      {hiddenMemberCount === 1 ? 'member' : 'members'}
                    </span>
                    <span aria-hidden="true">›</span>
                  </Link>
                ) : null}
              ...
              <p className={dashboardStyles.accessRestricted}>
                Member details are available to World managers.
              </p>
            )}
          </section>
        </div>
